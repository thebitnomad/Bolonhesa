...
class YT {
  ...
  static getVideoID = (url) => {
    if (!this.isYTUrl(url)) throw new Error("URL informada não é um link válido do YouTube.");
    ...
    try {
      ...
      stream.on("progress", (chunkLength, downloaded, total) => {
        ...
        process.stdout.write(
          `(${(downloaded / 1024 / 1024).toFixed(2)}MB de ${(
            total /
            1024 /
            1024
          ).toFixed(2)}MB)\n`
        ...
        process.stdout.write(
          `tempo decorrido: ${downloadedMinutes.toFixed(2)} minutos`
        );
        process.stdout.write(
          `, tempo estimado restante: ${estimatedDownloadTime.toFixed(2)} minutos `
        );
        readline.moveCursor(process.stdout, 0, -1);
        // let txt = `${bgColor(color('[FFMPEG]]', 'black'), '#38ef7d')} ${color(moment().format('DD/MM/YY HH:mm:ss'), '#A1FFCE')} ${gradient.summer('[Convertendo..]')} ${gradient.cristal(p.targetSize)} kb`
      });
