module.exports = async (context) => {
  ...
  if (!text || text.trim() === "") {
    return m.reply(
      formatStylishReply(
        `Envie um texto para eu criar o logo *Glossy Silver*.\n\nExemplo: !glossysilver Seu Texto`
      )
    );
  }
  ...
  try {
    ...
    const data = await fetchJson(
      `https://api.giftedtech.web.id/api/ephoto360/glossysilver?apikey=gifted&text=${encodedText}`
    );

    if (data && data.success && data.result && data.result.image_url) {
      const caption = formatStylishReply(
        `Aqui está o seu logo *Glossy Silver*, ${userName}! ✨\n\n📸 *Texto:* ${cleanedText}\n🔗 *Fonte:* Ephoto360\n🤖 Gerado por *${botname}*`
      );
    ...
    await m.reply(
      formatStylishReply(
        `Ocorreu um erro ao gerar o logo *Glossy Silver*, ${userName}.\n\nDetalhes: ${error.message}`
      )
    );
  }
};
